...
/**
 * Enhanced Fines Panel – uses configurable fine rate and notifies other panels.
 */
public class FinesPanel extends JPanel {
    ...
    public FinesPanel() {
        ...
        tabbedPane.addTab("💰 Fines Management", createFinesPanel());
    ...
    private JPanel createFinesPanel() {
        ...
        refreshBtn.addActionListener(e -> { loadFines(); updateStats(); });
        ...
        finesTable.setDefaultRenderer(Object.class, new javax.swing.table.DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                if (!isSelected) {
                    ...
                    if ("UNPAID".equals(status)) c.setBackground(new Color(255, 230, 230));
    ...
    private JPanel createOverduePanel() {
        ...
        double rate = getFineRate();
        JLabel infoLabel = new JLabel(String.format(
                "<html><b>Note:</b> These books are currently overdue. " +
                        "Fines accrue at K%.2f per day. Use 'Calculate Overdue Fines' to create fine records.</html>", rate));
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                if (!isSelected) {
                    try {
                        ...
                        if (daysOverdue > 14) c.setBackground(new Color(255, 200, 200));
                        else if (daysOverdue > 7) c.setBackground(new Color(255, 230, 200));
        ...
        refreshOverdueBtn.addActionListener(e -> { loadOverdueBooks(); updateStats(); });
    ...
    private void loadOverdueBooks() {
        ...
        try (Connection conn = DBHelper.getConnection()) {
            ...
            while (rs.next()) {
                ...
                double estimatedFine = daysOverdue * rate;
    ...
    private void updateStats() {
        ...
        try (Connection conn = DBHelper.getConnection()) {
            ...
            if (rs.next()) totalUnpaidLabel.setText("K " + String.format("%.2f", rs.getDouble(1)));

            // Total paid
            sql = "SELECT COALESCE(SUM(amount), 0) FROM fines WHERE paid = TRUE";
            rs = conn.createStatement().executeQuery(sql);
            if (rs.next()) totalPaidLabel.setText("K " + String.format("%.2f", rs.getDouble(1)));

            // Estimated dues: SUM(days_overdue * rate) for overdue books without fines
            sql = "SELECT COALESCE(SUM(days_overdue * ?), 0) FROM (" +
                    "SELECT DATEDIFF(CURRENT_DATE, bb.due_date) AS days_overdue " +
                    "FROM borrowed_books bb " +
            ...
            if (rs.next()) totalDuesLabel.setText("K " + String.format("%.2f", rs.getDouble(1)));
    ...
    private void handleCalculateFines() {
        int confirm = JOptionPane.showConfirmDialog(this,
                "This will automatically calculate and create fine records for all overdue books.\n" +
                        "Fine rate: K" + String.format("%.2f", getFineRate()) + " per day.\n\n" +
                        "Do you want to continue?",
                "Calculate Fines", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);

        if (confirm != JOptionPane.YES_OPTION) return;

        try {
            FineCalculator.calculateOverdueFines();
            JOptionPane.showMessageDialog(this, "✅ Fines calculated successfully!\n\nRefreshing data...",
                    "Success", JOptionPane.INFORMATION_MESSAGE);
            loadFines();
            loadOverdueBooks();
            updateStats();
            RefreshManager.getInstance().notifyRefresh(RefreshManager.PANEL_FINES);
    ...
    private void handleMarkPaid() {
        ...
        try (Connection conn = DBHelper.getConnection()) {
            PreparedStatement stmt = conn.prepareStatement(
                    "UPDATE fines SET paid = TRUE, payment_date = CURRENT_DATE WHERE id = ?");
    ...
    private void handleCreateFine() {
        // Placeholder – actual fine creation for a single record can be added later.
        JOptionPane.showMessageDialog(this,
                "Select 'Calculate Overdue Fines' to create fines in bulk.");
    }
    ...
    private void handleContactMember() {
        ...
        JOptionPane.showMessageDialog(this,
                "Contact Member\n\nName: " + member + "\nEmail: " + email + "\nPhone: " + phone +
                        "\n\nOverdue Book: " + book + "\n\n(Email/SMS integration not yet implemented)");
    }

    private void handleViewFineDetails() {
        // Placeholder – can be implemented similarly to other detail views.
        JOptionPane.showMessageDialog(this, "Fine details view (to be implemented).");
    }

    private void handleSendReminder() {
        // Placeholder
        JOptionPane.showMessageDialog(this, "Sending reminder... (feature not yet available)");
    }

    private void exportFinesToPDF() {
        // Placeholder – PDF export similar to other panels.
        JOptionPane.showMessageDialog(this, "Exporting to PDF... (feature not yet available)");
    }

    // ===== HELPER =====
    private double getFineRate() {
        return FineCalculator.getFinePerDay();
    }
}